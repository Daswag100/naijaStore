...
export default function AdminOrderDetailsPage() {
    ...
    try {
      ...
      if (response.ok && data.orders) {
        ...
        if (foundOrder) {
          setOrder({
            ...foundOrder,
            order_items: foundOrder.order_items.map((item: any) => ({
              ...item,
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'processing': return <Badge className="bg-purple-100 text-purple-800">Processing</Badge>;
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-sm space-y-1">
                <p className="font-medium">{order.shipping_address?.name || order.customer.name}</p>
